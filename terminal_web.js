...
var wasmBinaryFile;
  wasmBinaryFile = 'terminal_web.wasm';
...
function js_beep(freq,ms){ if (!window._audioCtx) window._audioCtx = new AudioContext(); const ctx = window._audioCtx; const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.frequency.value = freq; osc.connect(gain); gain.connect(ctx.destination); osc.start(); gain.gain.setValueAtTime(1, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + ms / 1000); osc.stop(ctx.currentTime + ms / 1000); }
function js_drawRect(x,y,w,h,color){ if (!Module.canvasCtx) return; const ctx = Module.canvasCtx; ctx.strokeStyle = "lime"; ctx.strokeRect(x, y, w, h); }
function js_drawString(cstr,x,y){ if (!Module.canvasCtx) return; const ctx = Module.canvasCtx; const text = UTF8ToString(cstr); ctx.fillStyle = "lime"; ctx.font = "12px monospace"; ctx.fillText(text, x, y); }
function js_fillRect(x,y,w,h,color){ if (!Module.canvasCtx) return; const ctx = Module.canvasCtx; ctx.fillStyle = "black"; if (color == 1) ctx.fillStyle = "lime"; ctx.fillRect(x, y, w, h); }
function js_fillScreen(color){ if (!Module.canvasCtx) return; const ctx = Module.canvasCtx; ctx.fillStyle = "black"; ctx.fillRect(0, 0, Module.canvas.width, Module.canvas.height); }
function js_getFlag(which){ if (!Module._kbState) return 0; const st = Module._kbState; switch (which) { case 0: return st.enter|0; case 1: return st.del|0; case 2: return st.tab|0; case 3: return st.ctrl|0; } return 0; }
function js_getWord(out,maxLen){ if (!Module._kbState) { out[0] = 0; return; } const w = Module._kbState.word || ""; const len = Math.min(w.length, maxLen-1); for (let i=0; i<len; ++i) { Module.HEAPU8[out + i] = w.charCodeAt(i); } Module.HEAPU8[out + len] = 0; }
function js_hasInputChange(){ return Module._kbChanged || 0; }
function js_popState(){ Module._kbChanged = 0; return 1; }
...
var asmLibraryArg = {
  ...
  "fd_write": _fd_write,
  "js_beep": js_beep,
  "js_drawRect": js_drawRect,
  "js_drawString": js_drawString,
  "js_fillRect": js_fillRect,
  "js_fillScreen": js_fillScreen,
  "js_getFlag": js_getFlag,
  "js_getWord": js_getWord,
  "js_hasInputChange": js_hasInputChange,
  "js_popState": js_popState,
